...
const CATEGORY_CODE = "warehouse";

const descriptions: Record<string, string> = {
  incoming: "Приход материалов или товаров на склад.",
  outgoing: "Расход материалов или товаров со склада.",
  reserve: "Резервирование остатков под заказ или производство.",
  writeoff: "Списание остатков со склада.",
  draft: "Документ создан, но ещё не проведён и не влияет на склад.",
  posted: "Документ проведён и влияет на складские остатки.",
...
function getDescription(status: StatusItem) {
  return descriptions[status.code] || "Пользовательский складской статус.";
}
...
export default function WarehouseStatusesPage() {
  ...
  async function loadStatuses() {
    ...
    if (!categoryData) {
      setError('Категория "warehouse" не найдена в status_categories');
      setLoading(false);
      return;
    }
  ...
  function openCreate() {
    ...
    setStatusSortOrder(sortedStatuses.length + 1);
    setIsCreateOpen(true);
    setMessage("");
    setError("");
  }

  function openStatus(status: StatusItem, index: number) {
  ...
  async function createStatus() {
    if (!category) {
      setError("Категория складских статусов не найдена");
      return;
    }
  ...
  async function moveStatus(currentIndex: number, direction: "up" | "down") {
  ...
  return (
    <div style={styles.page}>
      ...
      {loading && <div style={styles.notice}>Загрузка складских статусов...</div>}
      ...
      <div style={styles.section}>
        <div style={styles.sectionHeader}>
          <div style={styles.sectionTitleRow}>
            ...
            <div>
              <div style={styles.sectionTitle}>Складские статусы</div>
              <div style={styles.sectionText}>
                Workflow складских движений, резервов, списаний и приходов
              </div>
            </div>
          </div>
        ...
        <div style={styles.statusList}>
                ...
                <div style={styles.statusBottomRow}>
                  ...
                  >
                    ...
                    <button
                      style={{
                        ...styles.orderButton,
                        opacity: index === sortedStatuses.length - 1 ? 0.35 : 1,
                        cursor:
          ...
          {sortedStatuses.length === 0 && (
            <div style={styles.emptyBox}>
              Складские статусы пока не добавлены
            </div>
          )}
        </div>
      </div>
      ...
      {isCreateOpen && (
        <div style={styles.modalOverlay} onClick={() => setIsCreateOpen(false)}>
          <div style={styles.modal} onClick={(event) => event.stopPropagation()}>
            <div style={styles.modalHeader}>
              <div>
                <div style={styles.modalTitle}>Новый складской статус</div>
                <div style={styles.modalSubtitle}>Добавление статуса в workflow склада</div>
              </div>
...
function StatusForm({
  ...
  statusColor: string;
  setStatusColor: (value: string) => void;
  statusSortOrder: number;
  setStatusSortOrder: (value: number) => void;
  statusKind: StatusKind;
  setStatusKind: (value: StatusKind) => void;
}) {
  return (
